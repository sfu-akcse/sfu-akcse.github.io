(function registerEventsSection(global) {
  const AKCSE = (global.AKCSE = global.AKCSE || {});
  const components = (AKCSE.components = AKCSE.components || {});
  const { useState } = global.React;

  components.EventsSection = function EventsSection({ copy, years }) {
    ...
    const renderEventCard = (event) => {
      ...
      return (
        <article className="event-card" key={event.key}>
        ...
        <div className="section__container">
          ...
          <div className="timeline">
            {years.map(({ year, events }) => {
              ...
              return (
                <div className={`event-dropbar ${isOpen ? 'is-open' : ''}`} key={year}>
                  <button
                    type="button"
