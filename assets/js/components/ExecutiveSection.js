(function registerExecutiveSection(global) {
  const AKCSE = (global.AKCSE = global.AKCSE || {});
  const components = (AKCSE.components = AKCSE.components || {});

  components.ExecutiveSection = function ExecutiveSection({ copy, members, language }) {
      ...
      <section id="executive" className="section">
        <div className="section__container">
          ...
          <div className="grid grid--executive" role="list">
            {members.map((member) => (
              <article className="card" role="listitem" key={member.name}>
                <img
